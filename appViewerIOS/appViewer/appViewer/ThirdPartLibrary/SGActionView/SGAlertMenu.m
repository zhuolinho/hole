...
- (void)setupWithTitle:(NSString *)title message:(NSString *)message actionTitles:(NSArray *)actionTitles;
{
    ...
    if (message) {
        ...
        //jiangjin
        UIImageView * l_image = [[UIImageView alloc]initWithFrame:CGRectMake(_messageLabel.frame.origin.x, _messageLabel.frame.origin.y, 10, 10)];
        l_image.image = [UIImage imageNamed:@"plus"];
        [_messageLabel addSubview:l_image];
        //jiangjin
...
- (void)setStyle:(SGActionViewStyle)style{
    ...
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[SGButton class]]) {
            ...
            //jiangjin
            [(SGButton*)view setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [(SGButton*)view setBackgroundColor:[UIColor orangeColor]];
        }
    }
}
