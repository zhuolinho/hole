...
- (id)initWithFrame:(CGRect)frame items:(NSArray*)items iconPosition:(IconPosition)position andSelectionBlock:(selectionBlock)block{
    ...
    if (self) {
        ...
        for(NSString *item in items){
            ...
            UIButton *button =[[UIButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, frame.size.height) text:item icon:nil
                                               textAttributes:nil andIconPosition:position];
